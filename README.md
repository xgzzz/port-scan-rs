...
打开 GUI 后：
- **📊 抓包分析** — 打开 pcap 文件，实时显示协议统计、TCP 标志位、三次握手时间线、包列表
- **🖥️ 本机端口** — 列出本机监听端口
- **🌐 远程扫描** — 输入主机和端口，并发扫描
- **📡 抓包** — 两种模式：
  - **网卡抓包**（需 `--features pcap`）：刷新并选择网卡、填写 BPF 过滤表达式、开始/停止抓包；实时预览每个包的源/目的地址、协议、长度与摘要（HTTP 请求行、TCP 标志、载荷文本），支持显示过滤、自动滚动、清空，并可勾选保存为 pcap 文件；
